"""
Fast Evaluation Script - Runs minimal tests to generate results quickly.

This runs a very small subset of tests:
- 1 map (Town03)
- 1 weather condition (Clear)
- 3 modes (multimodal, vision_only, baseline)
Total: 3 tests (much faster than 12-test quick evaluation)

With only 100 frames per test, this should complete in ~5-10 minutes.
"""
...
# Minimal test configuration for fast evaluation
MAPS = ["Town03"]  # Single representative map
WEATHER_PRESETS = {
    "Clear": {
        "cloudiness": 0.0,
        "precipitation": 0.0,
...
# Very reduced frame count for fast testing
FRAMES_PER_TEST = int(os.environ.get("EVAL_FRAMES", "100"))  # Only 100 frames per test
...
def run_single_evaluation(map_name: str, weather_name: str, mode: str, 
    ...
    try:
        result = subprocess.run(
            ...
            timeout=180  # 3 minute timeout per test (reduced from 5)
        )
        ...
        if os.path.exists(result_file):
            try:
                with open(result_file, "r") as f:
                    ...
                    print(f"     Safety Rate: {metrics.get('safety_rate', 0):.2f}%")
        ...
        if result.returncode != 0:
            ...
            if len(output_lines) > 5:
                print("[ERROR] Last 5 lines of output:")
...
def run_fast_evaluation():
    """Run fast evaluation with minimal tests."""
    results = {
        ...
        "config": {
            ...
            "note": "Fast evaluation - minimal test suite (3 tests)"
        },
        "results": []
    }
    ...
    print("="*60)
    print("FAST EVALUATION - Minimal Test Suite")
    print("="*60)
    print(f"Total tests: {total_tests}")
    print(f"Frames per test: {FRAMES_PER_TEST}")
    print(f"Estimated time: ~{total_tests * 2} minutes")
...
if __name__ == "__main__":
    ...
    print("This will run 3 minimal tests (1 map × 1 weather × 3 modes).")
    print("Frames per test: 100 (very fast)")
    import time
